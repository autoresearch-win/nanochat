...
class TopKRouter(nn.Module):
    ...
    def forward(self, x):
        ...
        _, selected_experts = torch.topk(
            biased_scores, k=self.top_k, dim=-1, sorted=False
        )
    ...
    def forward(self, x, num_tokens_per_expert):
        ...
        if x.is_cuda:
            return _run_experts_grouped_mm(
                self.w_up, self.w_down, x, num_tokens_per_expert
            )
...
class MoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.shared_expert = (
            SharedExpert(dim, expert_hidden_dim) if num_shared > 0 else None
        )
    ...
    def forward(self, x):
        ...
        shared_output = (
            self.shared_expert(x_flat) if self.shared_expert is not None else None
        )
